...
<body class="bg-slate-950 text-slate-100 min-h-screen">
    ...
    <nav class="bg-slate-900 border-b border-slate-800 px-6 py-4 flex flex-wrap justify-between items-center gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold text-white leading-tight">SmartStaff Admin Portal</h1>
    ...
    <main class="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-900/50 via-indigo-900/40 to-slate-900 border border-blue-800/40 rounded-3xl p-6 flex flex-wrap justify-between items-center gap-4">
            ...
            <div class="flex gap-4 text-center">
                <div class="bg-slate-800/70 border border-slate-700 px-5 py-3 rounded-2xl">
                    <p class="text-xs text-slate-400">የገቡ (Present)</p>
                    <p class="text-2xl font-bold text-emerald-400">{{ $presentCount }}</p>
                </div>
                <div class="bg-slate-800/70 border border-slate-700 px-5 py-3 rounded-2xl">
                    <p class="text-xs text-slate-400">ያረፈዱ (Late)</p>
                    <p class="text-2xl font-bold text-amber-400">{{ $lateCount }}</p>
                </div>
                <div class="bg-slate-800/70 border border-slate-700 px-5 py-3 rounded-2xl">
                    <p class="text-xs text-slate-400">ያልገቡ (Absent)</p>
                    <p class="text-2xl font-bold text-rose-400">{{ $absentCount }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                <div class="flex justify-between items-center mb-5">
                    <h3 class="text-base font-bold text-white flex items-center gap-2">
                        📍 የዛሬ የቀጥታ አቴንዳንስ (Live Attendance)
                    </h3>
                    ...
                    <table class="w-full text-left text-xs">
                        <thead class="bg-slate-800/60 text-slate-400 uppercase text-[10px]">
                            <tr>
                                ...
                                <th class="p-3">መግቢያ (In)</th>
                                <th class="p-3">ርቀት (GPS)</th>
                                <th class="p-3">መውጫ (Out)</th>
                                <th class="p-3">ሁኔታ</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-800">
                            @forelse($todayAttendances as $att)
                                <tr class="hover:bg-slate-800/40 transition">
                                    ...
                                    <td class="p-3 font-mono text-slate-300">{{ $att->check_in_distance_meters }} ሜትር</td>
            ...
            <!-- Geofence & Quick Controls -->
            <div class="space-y-6">

                <!-- 100m GPS Settings Form -->
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                    <h3 class="text-sm font-bold text-white mb-3 flex items-center gap-2">
                        🌐 የ 100 ሜትር ጂፒኤስ አጥር ቅንብር
                    </h3>
                    <form action="{{ route('admin.geofence.update') }}" method="POST" class="space-y-3 text-xs">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-slate-400 mb-1">ኬክሮስ (Latitude)</label>
                            ...
                            <div>
                                <label class="block text-slate-400 mb-1">ኬንትሮስ (Longitude)</label>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div>
                                <label class="block text-slate-400 mb-1">የመግቢያ ሰዓት</label>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                    <h3 class="text-sm font-bold text-white mb-3 flex items-center gap-2">
                        📢 ለሰራተኞች መልእክት ማስተላለፊያ
                    </h3>
                    <form action="{{ route('admin.announcement.post') }}" method="POST" class="space-y-3 text-xs">
                        @csrf
                        <div>
                            <input type="text" name="title" placeholder="የማስታወቂያው ርዕስ..." required class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-white">
                        </div>
                        <div>
                            <textarea name="message" rows="2" placeholder="መልእክትዎን እዚህ ይጻፉ..." required class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-white"></textarea>
                        </div>
                        <div class="flex justify-between items-center">
                            ...
                            <button type="submit" class="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 font-bold text-white rounded-lg transition">
                                አስተላልፍ
                            </button>
                        </div>
                    </form>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @forelse($pendingLeaves as $leave)
                    <div class="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-4 space-y-3">
                        ...
                        <form action="{{ route('admin.leave.status', $leave->id) }}" method="POST" class="flex gap-2 pt-2 border-t border-slate-700">
                            @csrf
